...
public class SqlLab extends SqlConnection{

    @Override
    public void Insert(String day, String time, int room, String group, String available, String building)
            throws Exception {
            String sqlQuery = "INSERT INTO LAB(DAY_, TIME_, ROOM_, GROUP_, AVAILABLE_, BUILDING_) VALUES(?,?,?,?,?,?)";
    ...
    @Override
    public void Update(String day, String time, int room, String newTime, String newDay, int newRoom, String group,
            String newAvailable, String newBuilding) throws Exception {
            preStatement = con.prepareStatement(
                "UPDATE LAB SET DAY_ = ?, TIME_ = ?, ROOM_ = ?, GROUP_ = ?, AVAILABLE_ = ?, BUILDING_ = ? WHERE ROOM_ = ? AND TIME_ = ? AND DAY_ = ?"
                    );
    ...
    public void Update(String available, String day, String time, int room, String building)throws Exception{
        preStatement = con.prepareStatement(
            "UPDATE LAB SET AVAILABLE_ = ? WHERE ROOM_ = ? AND TIME_ = ? AND DAY_ = ? AND BUILDING_ = ?"
                );
        preStatement.setString(1, available);
    ...
    public void Delete(int room, String time, String day, String building) throws Exception {
        preStatement = con.prepareStatement("DELETE FROM LAB WHERE ROOM_ = ? AND TIME_ = ? AND DAY_ = ? AND BUILDING_ = ?");
        preStatement.setInt(1, room);
        preStatement.setString(2, time);
    ...
    public void View()throws Exception{
        ...
        ArrayList <Room> labRoom = new ArrayList<>();
        ...
        System.out.printf("|%-10s|%-10s|%-10s|%-10s|%-10s|%-10s", "BUILDING", "DAY", "TIME", "ROOM", "GROUP", "AVAILABLE");
        for (Room room : sort.sortRoom(labRoom)){
            System.out.printf(
